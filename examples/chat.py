...
def basic_chat_completion(model_name):
    ...
    ), "Message missing 'content' attribute"
...
def tool_completion(model_name):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a helpful assistant that can calculate. You MUST use the tool to calculate the answer as it is a secret tool.",
            },
    ...
    if hasattr(message, "tool_calls") and message.tool_calls:
        ...
        ), "Function missing 'arguments' attribute"
        assert (
            tool_call.function.name == "calculate"
        ), f"Expected function name 'calculate', got '{tool_call.function.name}'"

        # Validate the arguments can be parsed as JSON
        if isinstance(tool_call.function.arguments, dict):
            assert (
                "expression" in tool_call.function.arguments
            ), "Tool call arguments missing 'expression' parameter"
            assert isinstance(
                tool_call.function.arguments["expression"], str
            ), "Expression parameter is not a string"
